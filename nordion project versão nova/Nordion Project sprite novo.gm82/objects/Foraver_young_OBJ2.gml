...
ds_list_add(falas, "CG01;Rvl:oque????? como ela chegou tao rapido aqui");
ds_list_add(falas, "CG01;Charlotte:o senhora fatasma Nimbus vai ficar pra tras");
ds_list_add(falas, "CG02;Rvl:Merda!!!!como eu nao vi ela???? ela tem um cabelo
bem visvel durante a noite, afinal nos duas somos ruivas");
ds_list_add(falas, "CG03;Rvl:e isso ela usou o terno dela para tampar o cabelo
e ficar mas aerodinamica");
ds_list_add(falas, "CG04;Rvl:bem eu vou deixar ela ganhar afinal nao quero
maltratar o supra e sem contar que uma mulher bonita como ela merese a vitoria");
ds_list_add(falas, "CG04;Charlotte:Revolution isso e incesto");
...
applies_to=self
*/
// 1. Definição dos ganhos (Lucro Bruto fixo de 2375 / 3 = 791)
lucro_bruto = floor(2375 / 3);
imposto_ind = floor(lucro_bruto * 0.40); // 40% de imposto
taxa_clone  = floor(lucro_bruto * 0.35); // 35% de taxa

// Lucro líquido que sobra (aprox. 197)
lucro_liquido = lucro_bruto - imposto_ind - taxa_clone;

// 2. MULTA ESTILO JAPÃO (Excesso de Velocidade Grave)
// Baseado em ¥100.000 (cem mil ienes), valor padrão para infrações graves.
multa_japao = 546;

// 3. Aplicação no Saldo Global
global.money += lucro_liquido;   // Adiciona o lucro da corrida
global.money -= multa_japao;     // Subtrai a multa pesada (pode deixar negativo)

// 4. Exibição da Mensagem formatada
msg = "!!! YOU WIN !!!" + "\n" +
          "---------------------------" + "\n" +
          "Ganhos da corrida: +" + string(lucro_liquido) + "\n" +
          "Multa (Padrão FatesLandia): -" + string(multa_japao) + "\n" +
          "---------------------------" + "\n" +
